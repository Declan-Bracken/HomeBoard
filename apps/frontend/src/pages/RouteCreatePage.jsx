...
function renderCanvas(imageCanvas, overlayCanvas, img, holds, holdRolesRef, state) {
  ...
  // Image layer — draw darkened version as base
  ...
  for (const hold of holds) {
    ...
    if (role) {
      ...
      // Cut out hold from darkening on image canvas
      ic.save()
      ...
      // Colored overlay on selected hold
...
function RouteCanvas({ imageUrl, holds, holdRolesRef, onHoldClick, imageWidth, imageHeight, scheduleRenderRef }) {
  ...
  // Expose scheduleRender to parent so it can trigger re-render on hold click
  useEffect(() => {
    if (scheduleRenderRef) scheduleRenderRef.current = scheduleRender
  }, [scheduleRender, scheduleRenderRef])

  useEffect(() => {
    downsampleImage(imageUrl, 1200).then(img => {
      S.current.img = img
      scheduleRender()
    })
  ...
  }, [holds, onHoldClick, scheduleRender])
...
export default function RouteCreatePage() {
  ...
  // holdRoles lives in a ref — changes don't cause re-renders or remount the canvas
  const holdRolesRef = useRef({})
  // UI-only state for sidebar counts
  ...
  const { data: wall, isLoading: wallLoading } = useQuery({
    queryKey: ['wall', id],
    queryFn: async () => {
      const res = await api.get(`/walls/${id}/`)
      return res.data
    }
  })
  ...
  const { data: holds, isLoading: holdsLoading } = useQuery({
    queryKey: ['holds', id],
    queryFn: async () => {
      const res = await api.get(`/walls/${id}/holds/`)
      return res.data
    }
  })
  ...
  const handleHoldClick = useCallback((holdId) => {
    ...
    // Re-render canvas without remounting
            ...
            <>
              <div className="rc-canvas-col">
                <div className="rc-canvas-header">
                  ...
                  <span className="rc-hint">Click holds to assign roles · Scroll to zoom · Drag to pan</span>
                </div>
